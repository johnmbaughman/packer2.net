...
namespace Dean.Edwards {
    ...
    public class ParseMaster {
        ...
        private string DELETE(Match match, int offset) {
            return string.Format("\x01{0}\x01", match.Groups[offset].Value);
        }
        ...
        private Regex getPatterns() {
            ...
            foreach (object pattern in patterns) {
                rtrn.Append(string.Format("{0}|", ((Pattern)pattern).ToString()));
            }
        ...
        private string replacementString(Match match, int offset, string replacement, int length) {
            while (length > 0) {
                replacement = replacement.Replace(string.Format("${0}", length--), match.Groups[offset + length].Value);
            }
            return replacement;
        }
